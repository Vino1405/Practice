def process_position_properties(dom_tree):
    """Enhanced grid position processor with proper implicit grid handling"""
    ...
    def _process_node(node, parent_grid=None, is_first_child=False):
        ...
        for i, child in enumerate(node.get('component', [])):
    ...
    def _calculate_grid_position(node, parent_grid, current_grid, is_first_child):
        ...
        if parent_grid:
            max_cols = parent_grid.get('max_columns')
           
            # Handle implicit grids (max_cols is None)
            if max_cols is None:
                # For implicit grids, use infinite columns in row flow
                if parent_grid['auto_flow'] == 'row':
                ...
                # Handle explicit grids
                if parent_grid['auto_flow'] == 'column':
                    if parent_grid['columns'] > max_cols:
                        parent_grid['columns'] = 1
                        parent_grid['rows'] += 1
                    grid['pos-row'] = parent_grid['rows']
                    grid['pos-col'] = parent_grid['columns']
    ...
    def _get_max_columns(grid):
        """Extracts column count from grid properties"""
        ...
        # No explicit columns - implicit grid
        return None
